...
import pymorphy2
import scripthelper
startTimeLoad=int(time.time()*1000)
morph = pymorphy2.MorphAnalyzer(lang='ru')
finishTimeLoadPymorphy=int(time.time()*1000)
nowPath = os.getcwd()
#print(nowPath)
parPath=nowPath
...
finishTimeLoadRusentilex=int(time.time()*1000)
print("speedTest:Loading:pymorphy2="+str(finishTimeLoadPymorphy-startTimeLoad)+"ms||rusentilex="+str(finishTimeLoadRusentilex-finishTimeLoadPymorphy)+"ms")
...
lemmes= []
word1=""
startTimeUsage=int(time.time()*1000)
for word in words:
    #i=i+1
    #if(i==500):
    #    break
    ...
    if(RuSentiLexLoaded==0):
        continue
finishTimeUsagePymorphy=int(time.time()*1000)
for lemma in lemmes:
    if(len(lemma)<4):
        index=-1
    else:
        index=scripthelper.quickFindInSortedList(lemma,RuSentiLexVocabulary)
    if(index==-1):
    ...
    if(scripthelper.cmpstrCharIndex(lemma,RuSentiLexVocabulary[index])!=-1):
...
finishTimeUsageRusentilex=int(time.time()*1000)
f.close()
o1.close()
o2.close()
print("speedTest:Usage:Pymorphy2="+str(finishTimeUsagePymorphy-startTimeUsage)+"ms||Rusentilex="+str(finishTimeUsageRusentilex-finishTimeUsagePymorphy)+"ms")
if(RuSentiLexLoaded):
