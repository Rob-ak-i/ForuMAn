...
public class SyntaxProcessorTables {
	...
	protected static boolean isMulticatchTag(OpenCorporaTag newTag, boolean isNewTagParent) {
		int index= newTag.tag_POS;
		int[] multicatchTable = multicatchChildSocket;
		if(isNewTagParent) multicatchTable = multicatchParentSocket;
		if(multicatchTable[index]==multicatchAllwaysEnabled)return true;
		if(isEnumeration() && multicatchTable[index]==multicatchIfEnumeration) return true;
		return false;
	}
	...
	/**newTag become parent to stackedTag. Usage: [newTag][stackedTag]*/
	...
	/**newTag become child to stackedTag. Usage: [newTag][stackedTag]*/
	...
	protected static char getOperation(OpenCorporaTag newTag, OpenCorporaTag stackedTag, boolean isNewTagParent) {
		int newTagIndex=-1;
		int stackedTagIndex=-1;
		newTagIndex=getTableIndex(newTag);
		stackedTagIndex=getTableIndex(stackedTag);
		if(isNewTagParent)
			return connectionParentConditions[newTagIndex][stackedTagIndex];
		else
			return connectionChildConditions[newTagIndex][stackedTagIndex];
		
	} 
	protected static int getTableIndex(OpenCorporaTag tag) {
		int index=-1;
		index=tag.tag_POS;
		return index;
	}
