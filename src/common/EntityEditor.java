...
/**logic part of GUI*/
public class EntityEditor extends EntityEditorLowLevel {
	...
	public EntityEditor(int width, int height) {
		...
		binaryPartsContainer = SchematicManager.binaryParts;//new ArrayList<PartBinaryBasic>();
	...
	public void setPartName(PartBasic part, String name) {
		part.setName(name);
	}
	public PartBasic prepareBoundedElement(Point position) {
		int index;
		index = addNode(position);
		return elementsContainer.get(index);
	}
	public boolean createBoundedElement(Point position, Object linkedObject) {
		int index;
		index = addNode(position);
		CommonData.boundedObjectController.linkUnit(elementsContainer.get(index), linkedObject, true);
		return true;
	}
	...
	public boolean printDiscuss(Point position, double radiusScale, DataTable discuss, boolean isAppendMode) {
		if(isAppendMode) {System.out.println("Adding with append mode is not completed.");return false;}
		//TODO appendMode
		String forumKey = CommonData.tableManager.getManagedElementIdentifier(discuss);
		...
		double radius=discussUsersCount*radiusScale;
		double nowX, nowY;double angle=0,deltaAngle=Math.PI*2./(discussUsersCount+1);int userID;
		for(int i=0;i<discussUsersCount;++i) {
			userID=TextForum.getUserIDList(discuss).indexOf(uniqueUserIDs.get(i));
			tmpstr=TextForum.getUserName(discuss,userID);
			nowX=radius*Math.cos(angle)+position.x;
			nowY=radius*Math.sin(angle)+position.y;
			angle+=deltaAngle;
			index=addNode((int)nowX,(int) nowY);
			part=elementsContainer.get(index);
			part.setName(tmpstr);
			part.setProperty(Parameters.parameter_UnitClass, Parameters.parameter_UnitClass_user);
			part.setProperty(Parameters.parameter_ID, Integer.toString(userID));
			part.setProperty(Parameters.parameter_ForumKey, forumKey);
		}
		...
		for(int i=0;i<discussUsersCount;++i)
			for(int j=0;j<i;++j) {
				...
				nowX=(n0.pos.x+n1.pos.y);nowX=nowX*0.5;
				nowY=(n0.pos.y+n1.pos.y);nowY=nowY*0.5;
	...
	public boolean printSequenceTree(Point position, Sequence rootSequence) {
		//int treeHeight=rootSequence.get(0);
		int width=rootSequence.getNumberOfLeaves();
		printSequenceSubTree(position.x,position.y+stepY,width*stepX,rootSequence);
		return true;
	}
	public boolean printMessagesSyntaxes(Point position, MessageBank messages) {
		...
		int totalwordsCount;
		int nowX=position.x,nowY=position.y,previousTreeHeight=0;
		totalwordsCount=0;
		int maxWordsAtPreviousMessage=0;int nowRadius=0;
		for(int i=0;i<messages.textMessages.size();++i) {
			nowY=position.y;nowX+=stepX+maxWordsAtPreviousMessage*2;
			totalwordsCount=0;
		...
		for(int i=0;i<messages.textMessages.size();++i) {
			totalwordsCount=0;
			nowY=position.y;nowX+=stepX*(previousTreeHeight+2);
			nowSentence=messages.textMessages.get(i);
			while(nowSentence!=null) {
				totalwordsCount+=nowSentence.words.size();
	...
	private int printSequenceSubTree(int x, int y, int subTreeWidth, Sequence rootNode) {
		...
		int nowY=y;
		int rootNodeIndex=this.addNode(nowX, nowY);
		PartBasic part=elementsContainer.get(rootNodeIndex);
		part.setName(rootNode.toText(CommonData.textManager.textLemmes));
		part.setProperty(Parameters.parameter_UnitClass, Parameters.parameter_UnitClass_sequence);
		CommonData.boundedObjectController.linkUnit(part, rootNode, false);//elementsContainer.get(rootNodeIndex).
	...
	public void drawElementOnMatrix(PartBasic element, boolean erase, int camposx, int camposy, int radiusx, int radiusy) {
		...
		if(!erase) {
			...
			else				captionPart=binaryPartsContainer.indexOf((PartBinaryBasic)element);
		}
	...
	public int addNode(Point position) {
		return addNode(position.x, position.y);
	}
	/**returns index in elementsContainer*/
	public int addNode(int xPad, int yPad) {
		while(partsCollider(xPad,yPad, 5))yPad+=5;
	...
	public void removeElements(ArrayList<PartBasic> elements) {
		removeElements(elements, false);
	}
	private void removeElements(ArrayList<PartBasic> elements, boolean reIndexPhysicalLayerLabels) {
		...
		//if(reIndexPhysicalLayerLabels)SchematicManager.reindexAllObjects();
	...
	public void removeElement(PartBasic element) {
		removeElement(element,false);
	}
	private void removeElement(PartBasic element, boolean reIndexPhysicalLayerLabels) {
	...
	private void removeNode(PartNode node, boolean reIndexPhysicalLayerLabels) {
		ArrayList<PartBinaryBasic> parts = 
				...
				CommonData.scheme.binaryPartsContainer;
	...
	public ArrayList<PartBinaryBasic> getPartsFromNode(PartNode node){
		...
		for(int i=0;i<binaryPartsContainer.size();++i) {
			if(
					(binaryPartsContainer.get(i).getNodeFrom()==node)
					||(binaryPartsContainer.get(i).getNodeTo()==node)
					)
				result.add(binaryPartsContainer.get(i));
		}
		return result;
	}
	...
	public void saveResult(String fileName) {
	    ...
	    try {
			...
			out.println(binaryPartsContainer.size());
			...
			for(int i=0;i<nodesContainer.size();++i) {
				...
				out.print(Integer.toString(i)
						+" "
						+Integer.toString(nodetemp.pos.x)
						+" "
						+Integer.toString(nodetemp.pos.y)
						+" "
						+nodetemp.getName()
						);
			...
			for(int i=0;i<binaryPartsContainer.size();++i) {
				parttemp=binaryPartsContainer.get(i);
				...
				out.print(
						...
						+" "
						+Integer.toString(parttemp.pos.x)
						+" "
						+Integer.toString(parttemp.pos.y)
						);
				out.println(
						" "
						+parttemp.getName());
	...
	public void loadFromFile(String fileName, java.util.List<String> loadingParameters) {
	...
	public boolean loadResult(String fileName) {
	    ...
	    try {
			...
			int partsCount=in.nextInt();dummy=in.nextLine();//out.println(binaryPartsContainer.size());
			...
			PartBinaryBasic parttemp;PartNode node0=null, node1=null;int node0index,node1index;
