...
public class TextForum {
	...
	*/
	...
	public static int getUniqueUsersCount(DataTable discuss){
		int count=0;
		ArrayList userIDs = getUserIDList(discuss);
		ArrayList<Integer> uniques = new ArrayList<Integer> ();
		util.Lists.getUniqueArrayList(userIDs, uniques);
		count = uniques.size();
		uniques.clear();
		return count;
	}
	...
	public static void readHTMPage(String fileName, DataTable result) {
        ...
        loadValues(sb, result);
        result.name=StringUtils.getNameFromFileName(fileName);
		
	}
	private static void loadValues(StringBuilder text, DataTable dataTable) {
		...
		try {
			...
			/**
			System.out.print("{Error: IndexOutOfBounds at region [");
			System.out.print(valueBegin);
			System.out.print(":");
